...
class process_icinga2_aws_instances:
    ...
    def remove_all_aws_hosts(self):
        """Gets list of instances , iterates over the lists and removes all instances.
        Deploys new config if new instances are found
        """
        deploy_config = False
        ...
        for account in all_instances:
            for instance in all_instances[account]:
                if subprocess.call(["icingacli", "director", "host", "exists", instance['InstanceId']]) == 0 :
                    deploy_config = True
                    subprocess.call(["icingacli", "director", "host", "delete", instance['InstanceId']])
                    print "removed node " + instance['InstanceId']
        if deploy_config:
            subprocess.call(["icingacli", "director", "config", "deploy"])
